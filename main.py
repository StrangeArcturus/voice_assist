...
def main():
    ...
    assistant = VoiceAssistantWithAllSkills()
    recognizer = Recognizer()

    # настройка взаимосвязей между структурами
    translator.set_dependies(assistant)
    assistant.set_dependies(translator, person)
    recognizer.set_dependies(translator, assistant)
...
def execute_command_with_name(assistant: VoiceAssistantWithAllSkills, command_name: str, *args: list):
    """
    ...
    for key in commands.keys():
        if command_name in key:
            # commands[key](*args)
            assistant.__getattribute__(commands[key])(*args)
        else:
